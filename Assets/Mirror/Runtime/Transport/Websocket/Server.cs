...
namespace Mirror.Websocket
{
    public class Server
    {
        ...
        async Task ReceiveLoopAsync(WebSocket webSocket, CancellationToken token)
        {
            ...
            //BRP DEV Code
        //    if (webSocket.State == WebSocketState.CloseReceived || webSocket.State == WebSocketState.CloseSent || webSocket.State == WebSocketState.Closed){
           //     return;
          //  }
            //BRP Dev Code
            try
        ...
        public async void Send(int connectionId, ArraySegment<byte> segment)
        {
            ...
            {
                //BRP DEV'S CODE
                if (client.State == WebSocketState.CloseReceived || client.State == WebSocketState.Closed || client.State == WebSocketState.CloseSent)
                {
                    return;
                }
                //BRP DEV'S CODE
